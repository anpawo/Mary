...
-- Glados
-- File description:
-- This module contains unit tests for the Compiler component.
-}
...
convertLiteralSpec = describe "convertLiteral" $ do
  ...
  it "converts ArrLit to VmPreArray" $
    convertLiteral (ArrLit IntType [Lit $ IntLit 1, Lit $ IntLit 2]) `shouldBe` VmPreArray "int" [[Push (VmInt 1)], [Push (VmInt 2)]]
  it "converts StructLit to VmPreStruct" $
...
compileExpressionSpec = describe "compileExpression" $ do
  ...
  it "compiles a IfThenElse expression" $ do
    ...
    compileExpression cond `shouldBe` [Load "a",Load "b",Push (VmFunc "<"),Call,JumpIfFalse 4,Load "a",Ret,Push (VmBool False),JumpIfFalse 2,Load "b",Ret]
...
compilerSpec = describe "compiler" $ do
  it "compiler with main" $ do
    ...
    let expected = [("mul_add",[Store "c",Store "b",Store "a",Load "a",Load "b",Load "c",Push (VmFunc "*"),Call,Push (VmFunc "+"),Call,Ret]),("test_condition",[Store "b",Store "a",Load "a",Load "b",Push (VmFunc "<"),Call,JumpIfFalse 4,Load "a",Ret,Push (VmBool False),JumpIfFalse 2, Load "b",Ret]),("main",[Push (VmInt 1),Push (VmInt 2),Push (VmInt 3),Push (VmFunc "mul_add"),Call,Store "res",Load "res",Ret])]
  ...
  it "compiler without main" $ do
    ...
    let expected = [("mul_add",[Store "c",Store "b",Store "a",Load "a",Load "b",Load "c",Push (VmFunc "*"),Call,Push (VmFunc "+"),Call,Ret]),("test_condition",[Store "b",Store "a",Load "a",Load "b",Push (VmFunc "<"),Call,JumpIfFalse 4,Load "a",Ret,Push (VmBool False),JumpIfFalse 2,Load "b",Ret])]
