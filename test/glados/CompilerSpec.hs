...
compileSubExpressionSpec = describe "compileSubExpression" $ do
    ...
    it "compiles a FunctionCall with arguments" $
      compileSubExpression (FunctionCall "double" [Literal (IntLit 42), Literal (IntLit 2)]) `shouldBe` [Push (VmInt 42), Push (VmInt 2), PushEnv "double", Call]
    it "compiles a Literal" $
      compileSubExpression (Literal (StringLit "hello")) `shouldBe` [Push (VmString "hello")]

compileExpressionSpec :: SpecWith ()
compileExpressionSpec = describe "compileExpression" $ do
  it "compiles a SubExpression" $ 
    compileExpression (SubExpression (Literal (IntLit 42))) `shouldBe` [Push (VmInt 42)]
  it "compiles a Variable" $ 
    compileExpression (Variable (IntType, "x") (Literal (IntLit 10))) `shouldBe` [Push (VmInt 10), Store "x"]
  it "compiles a Return expression" $ 
    compileExpression (Return (Literal (StringLit "result"))) `shouldBe` [Push (VmString "result"), Ret]

compileExpressionsSpec :: SpecWith ()
compileExpressionsSpec = describe "compileExpressions" $ do
  it "compiles a list of expressions" $ 
    let expressions = [SubExpression (Literal (IntLit 1)), Variable (IntType, "x") (Literal (IntLit 2)), Return (Literal (StringLit "done"))]
